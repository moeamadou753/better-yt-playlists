    ...
    >
      <div className="overlay-panel" onClick={e => e.stopPropagation()}>
        ...
        <div className="overlay-header">
          {view === VIEW.ITEMS
            ? <button className="overlay-back" onClick={handleBack}>← Back</button>
            : <div />
          }
          <span className="overlay-title">
            {view === VIEW.PLAYLISTS ? '🎵 Your Playlists' : activePlaylist?.title}
          </span>
        ...
        <div className="overlay-body">
          ...
          {!loading && !error && view === VIEW.PLAYLISTS && (
            filteredPlaylists.length === 0
              ? <p className="overlay-hint">No playlists found</p>
              : filteredPlaylists.map(pl => (
                <div key={pl.id} className="overlay-row" onClick={() => handleOpenPlaylist(pl)}>
                  {pl.thumbnail && <img src={pl.thumbnail} className="overlay-thumb" />}
                  <div className="overlay-row-text">
                    <div className="overlay-row-title">{pl.title}</div>
                    <div className="overlay-row-meta">{pl.itemCount} videos</div>
                  </div>
                </div>
              ))
          )}

          {!loading && !error && view === VIEW.ITEMS && (
            filteredItems.length === 0
              ...
              : filteredItems.map(item => (
                <div
                  ...
                  onClick={() => window.open(`https://youtube.com/watch?v=${item.videoId}`, '_self')}
                >
                  {item.thumbnail && <img src={item.thumbnail} className="overlay-thumb" />}
                  <div className="overlay-row-text">
